...
${VIP_OM}    10.97.111.17
${VIP_LDAP}  10.97.111.25
${VIP_PROV}  10.97.111.26
${VIP_SITE}  10.97.111.27
...
Ping test to the PROV VIP
    [Tags]  external
    ${result} =     Run Process   ping ${VIP_PROV} -c 1   shell=True
    Log     ${result.stdout}
    Should Be Equal As Integers	${result.rc}  0       msg=PROV VIP (${VIP_PROV}) did not respond to ping

#Ping test to the SITE VIP
#    [Tags]  external
#    ${result} =     Run Process   ping ${VIP_SITE} -c 1   shell=True
#    Log     ${result.stdout}
#    Should Be Equal As Integers	${result.rc}  0       msg=SITE VIP (${VIP_SITE}) did not respond to ping
